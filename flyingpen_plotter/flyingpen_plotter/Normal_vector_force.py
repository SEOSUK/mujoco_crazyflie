...
class ROSDataBuffer(Node):
    def __init__(self):
        super().__init__("normal_vector_force")
        ...
        self.latest["tau_norm"] = 0.0
        self.latest["rxf_con_norm"] = 0.0
        self.latest["rxf_pure_norm"] = 0.0
        ...
        self.n_alg = np.full(3, math.nan, dtype=float)
        self.f_g = np.full(3, math.nan, dtype=float)
        self.n_gt = np.full(3, math.nan, dtype=float)
        self.force_dir = np.full(3, math.nan, dtype=float)
        self.drone_pos = np.full(3, math.nan, dtype=float)
        self.drone_quat = np.full(4, math.nan, dtype=float)
        self.ee_pos = np.full(3, math.nan, dtype=float)
        self.ee_offset_body = np.array(
            list(self.get_parameter("end_effector_offset").value),
            dtype=float,
        )
        ...
        self.get_logger().info("normal_vector_force started")
    ...
    def cb_consistency(self, msg: WrenchStamped) -> None:
        ...
        if not np.all(np.isfinite(tau_hat)) or not np.all(np.isfinite(r_cross_f)):
            return
        with self.lock:
            self.latest["tau_norm"] = float(np.linalg.norm(tau_hat))
            self.latest["rxf_con_norm"] = float(np.linalg.norm(r_cross_f))
    ...
    def cb_pure_mob(self, msg: WrenchStamped) -> None:
        ...
        if not np.all(np.isfinite(force_pure)):
            return
        with self.lock:
            if not np.all(np.isfinite(self.drone_quat)) or not np.all(np.isfinite(self.ee_offset_body)):
                return
            rot_wb = quat_xyzw_to_rotmat(
                self.drone_quat[0],
                self.drone_quat[1],
                self.drone_quat[2],
                self.drone_quat[3],
            )
            r_world = rot_wb @ self.ee_offset_body
            self.latest["rxf_pure_norm"] = float(np.linalg.norm(np.cross(r_world, force_pure)))
    ...
    def cb_packed_debug(self, msg: Float64MultiArray) -> None:
        ...
        with self.lock:
            ...
            self.n_f[:] = data[31:34]
            self.n_alg[:] = data[34:37]
            self.n_v[:] = data[40:43]
            self.f_g[:] = safe_unit(self.n_v.copy())
            self._update_quality_angles_locked()
    ...
    def _update_quality_angles_locked(self) -> None:
        ...
        self.latest["angle_n_f_deg"] = angle_deg(self.n_alg, self.n_gt)
        self.latest["angle_n_v_deg"] = angle_deg(self.f_g, self.n_gt)
...
class PlotWindow(QMainWindow):
    def __init__(self, rosbuf: ROSDataBuffer):
        super().__init__()
        self.rosbuf = rosbuf
        self.setWindowTitle("normal_vector_force")
        ...
        self.info_label = QLabel("Waiting for normal-vector force data...")
    ...
    def _build_internal_panel(self) -> QWidget:
        ...
        self.plot_force = self._make_plot("|Wf|", "force [N]", min_height=125)
        self.curve_force = self.plot_force.plot(name="|Wf|", pen=pg.mkPen((200, 100, 50), width=2))
        self.plot_force.setYRange(0.0, 0.1, padding=0.0)

        self.plot_mtau = self._make_plot("Torque vs r x F", "torque [N m]", min_height=125)
        self.curve_tau = self.plot_mtau.plot(name="tau", pen=pg.mkPen((120, 80, 220), width=2))
        self.curve_rxf_con = self.plot_mtau.plot(name="r X F_con", pen=pg.mkPen((220, 70, 45), width=2))
        self.curve_rxf_pure = self.plot_mtau.plot(name="r X F_pure", pen=pg.mkPen((40, 150, 75), width=2))
        self.plot_mtau.setYRange(0.0, 0.01, padding=0.0)

        self.plot_vel = self._make_plot("|Wv_c|", "vel [m/s]", min_height=125)
    ...
    def _build_quality_panel(self) -> QWidget:
        ...
        self.plot_ang_geo = self._make_plot("n_geo, n", "deg", min_height=quality_min_height)
        ...
        self.plot_ang_f = self._make_plot("n_alg, n", "deg", min_height=quality_min_height)
        self.curve_ang_f = self.plot_ang_f.plot(name="n_alg", pen=pg.mkPen((220, 50, 50), width=2))
        self.plot_ang_f.setYRange(0.0, 30.0, padding=0.0)

        self.plot_ang_v = self._make_plot("f_g, n", "deg", min_height=quality_min_height)
        self.curve_ang_v = self.plot_ang_v.plot(name="f_g", pen=pg.mkPen((50, 160, 70), width=2))
        self.plot_ang_v.setYRange(0.0, 30.0, padding=0.0)

        self.plot_ang_force = self._make_plot("f_ext, n", "deg", min_height=quality_min_height)
        self.curve_ang_force = self.plot_ang_force.plot(name="f_ext", pen=pg.mkPen((180, 120, 40), width=2))
    ...
    def update_all(self) -> None:
        ...
        self.curve_tau.setData(t_win, arr["tau_norm"][mask])
